...
class ApiPermissionCheck(MiddlewareMixin):
    ...
    def process_request(self, request):
        if request.path.startswith('/api/v1.0/accounts/login'):
            ...
            return
        # rgc 自定义取消权限校验
        if request.path.startswith('/api/v1.0/accounts/account_manage/user'):
            return
        if request.path.startswith('/api/v1.0/accounts/account_manage/role'):
            return
        if request.path.startswith('/api/v1.0/accounts/account_manage/dept'):
            return
        if request.path.startswith('/api/v1.0/accounts/account_manage/user_role'):
            return
