...
class LoonDept(BaseModel):
    ...
    def create_dept(self, name, leader, approver):
        if not name:
            raise ValueError('Dept must have an name')
        if not leader:
            raise ValueError('Dept must have an leader')
        if not approver:
            raise ValueError('Dept must have an approver')
        dept = LoonDept()
        dept.name = name
        dept.leader = leader
        dept.approver = approver
        dept.save()
        return dept
...
class LoonRole(BaseModel):
    ...
    def create_role(self, name, description):
        if not name:
            raise ValueError('Role must have an name')
        if not description:
            raise ValueError('Role must have an description')
        role = LoonRole()
        role.name = name
...
class LoonUser(AbstractBaseUser):
    ...
    def create_user(self, email, username, password=None, dep=0):
        ...
        user = LoonUser()
        user.email = LoonUserManager().normalize_email(email)
...
class LoonUserRole(BaseModel):
    ...
    def create_user_role(self, user_id, role_id):
        if not user_id:
            raise ValueError('UserRole must have an user_id')
        if not role_id:
            raise ValueError('UserRole must have an role_id')
        user_role = LoonUserRole()
